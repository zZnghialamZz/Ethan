...
namespace Ethan {
  ...
  void ImGuiProcess::EventCall(Event &event) {
    ...
    event.SetHandled(event.IsInCategory(EventCategory::MOUSE) & io.WantCaptureMouse);
    event.SetHandled(event.IsInCategory(EventCategory::KEYS) & io.WantCaptureKeyboard);
  }
