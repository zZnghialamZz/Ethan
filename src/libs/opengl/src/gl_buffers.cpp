...
static unsigned int BufferDataUsageToOpenGL(BufferDataUsage usage) {
  switch (usage) {
    case BufferDataUsage::STATIC:
      return GL_STATIC_DRAW;
    case BufferDataUsage::DYNAMIC:
      return GL_DYNAMIC_DRAW;
  }
}

/// --- GLVertexBuffer
GLVertexBuffer::GLVertexBuffer(BufferDataUsage usage) : data_usage_(usage) {
...
void GLVertexBuffer::SetData(const void* data, uint32_t size) {
  // NOTE(Nghia Lam): Bind Buffer to make sure we set data to the right vertex
  // buffer Will this impact the performace ?
  GLCALL(glBindBuffer(GL_ARRAY_BUFFER, vertexbufferID_));
  GLCALL(glBufferData(GL_ARRAY_BUFFER,
                      size,
                      data,
                      BufferDataUsageToOpenGL(data_usage_)));
}

void GLVertexBuffer::SetSubData(const void* data,
                                uint32_t size,
                                uint32_t offset) {
  GLCALL(glBindBuffer(GL_ARRAY_BUFFER, vertexbufferID_));
...
GLIndexBuffer::~GLIndexBuffer() { GLCALL(glDeleteBuffers(1, &indexbufferID_)); }
