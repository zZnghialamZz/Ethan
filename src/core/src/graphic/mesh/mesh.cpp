...
Mesh::Mesh(const Mesh& mesh)
    : mesh_vao_(mesh.GetVertexArray()), is_hide_(mesh.IsHide()) {}
...
/**
 * Create Quad base on given position and size. Assume that the quad is rendered
 * in this ordered:
 *
 * 3 - 2
 * |   |
...
Shared<Mesh> Mesh::CreateQuad(float x, float y, float width, float height) {
  ...
  // NOTE(Nghia Lam): BufferLayout: This order does matter for the data to be
  // layout.
  BufferLayout layout;
  ...
  Shared<Ethan::IndexBuffer> quad_ib =
...
Shared<Mesh> Mesh::CreateQuad(const glm::vec2& position,
                              const glm::vec2& size) {
...
Shared<Mesh> Mesh::CreateBatchMesh() {
  ...
  Shared<Ethan::IndexBuffer> batch_ib =
      IndexBuffer::Create(indices, Renderer2D::GetData().Storage.MaxIndices);
